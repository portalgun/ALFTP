...
7. **`NEW_SESSION`** — the core logic:
   ...
   - **`DOWNLD`** builds one big lftp script combining `set_opts`, `DLDR` (mirror commands per directory), and `DLFL` (pget commands per file) into a temp file, which `run_lftp_script` feeds to `lftp` on stdin (kept for inspection on failure or with `ALFTP_DEBUG=1`).
